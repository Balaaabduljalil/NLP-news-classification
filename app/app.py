from ml_models.model_package_ulmfit import ModelPackageULMFIT
...
model_ulmfit = ModelPackageULMFIT("/app/data", "ulmfit_model")
#model_svm = ModelPackageSK("/app/sklearn_models/saved_models", "svc_all.joblib", nlp)
model_logreg = ModelPackageSK("/app/sklearn_models/saved_models", "logregression_all.joblib", nlp)
...
def predict():
	...
	if request.method == 'POST':
		...
		if request.form['submit_button'] == 'Generate':
			...
			if len(text_in)==0 :
				return render_template('index.html', prediction={"ERROR": "Please enter a headline or generate one"})
			else:
				# Check if text_in is in inference data
				filter1 = data['text'].isin([text_in]) 
				true_value = data['category'][filter1].values[0] if filter1.any() else 'does not exist in the inference dataset'

				output = model_logreg.topk_predictions(text_in, 3)
				return render_template('index.html', prediction=output, news=str(text_in), real=str(true_value))


		elif request.form['submit_button'] == 'ULMFit':
			text_in = request.form['text']
			if len(text_in)==0 :
				return render_template('index.html', prediction={"ERROR": "Please enter a headline or generate one"})
			else:
				...
				output = model_ulmfit.topk_predictions(text_in, 3)
				return render_template('index.html', prediction=output, news=str(text_in), real=str(true_value))
